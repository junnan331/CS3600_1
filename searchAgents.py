...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            nexty = int(y + dy)
            if not self.walls[nextx][nexty]:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in corner:
                    nextState = ((nextx, nexty), corner + ((nextx, nexty),))
                else:
...
def cornersHeuristic(state, problem):
    ...
    while (len(visit) < len(corners)):
        heu = (None, 999999)
        for c in corners:
            if c not in visit:
                h = util.manhattanDistance(coor, c)
                if h < heu[1]:
                    heu = (c, h)
        retHeu += heu[1]
        visit = visit + (heu[0],)
...
def foodHeuristic(state, problem):
    ...
    for i in foodGrid.asList():
        for j in foodGrid.asList():
            dis = util.manhattanDistance(i, j) 
            if (dis > step):
                step = dis
                x = i
                y = j

    disX = util.manhattanDistance(position, x)
    disY = util.manhattanDistance(position, y)

    return min(disX, disY) + step
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        ret = search.breadthFirstSearch(problem)
        return ret
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        minDot = food[0]
        minDis = util.manhattanDistance(state, food[0])
        
        for dot in food:
            distance = util.manhattanDistance(state, dot)
            if distance < minDis:
